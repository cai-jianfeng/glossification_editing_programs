...
def create_editing_programs(data_path='../CSL_data/CSL-Daily.txt'):
    from extract_editoring_program import min_edit_program_parallel
    ...
    data_preprocess_file_name = '../CSL_data/CSL-Daily_editing_chinese.txt'
    ...
    with open(data_preprocess_file_name, 'w', encoding='utf-8') as f:
        ...
        for i, (origin_data, compress_edit_trajectory) in enumerate(zip(data_lines[1:], compress_edit_trajectorys_lines)):
            f.write('|'.join([origin_data.replace('\n', ''), compress_edit_trajectory]))
    print('create editing program succeed !')


def main():
    data_path = '../CSL_data/CSL-Daily.txt'
    create_editing_programs(data_path)
